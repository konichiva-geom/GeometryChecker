...
import utils.Utils.getArithmeticToString
...
class DivNotation(val numerator: MutableMap<Notation, Fraction>, val denominator: MutableMap<Notation, Fraction>) :
    ...
    override fun toString(): String {
        var numeratorString =
            if (numerator.size == 1) getArithmeticToString(numerator) else "(${getArithmeticToString(numerator)})"
        if(numeratorString.isEmpty())
            numeratorString = "1"
        val denominatorString =
            if (denominator.size == 1) getArithmeticToString(denominator) else "(${getArithmeticToString(denominator)})"
